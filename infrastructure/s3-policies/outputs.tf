...
output "access_point_domain_name" {
  ...
  value       = var.create_access_point ? "${aws_s3_access_point.app_uploads_access_point[0].name}-${data.aws_caller_identity.current.account_id}.s3-accesspoint.${var.access_point_vpc_id != "" ? "vpc-" : ""}${data.aws_caller_identity.current.region != "" ? data.aws_caller_identity.current.region : "us-east-1"}.amazonaws.com" : null
}
