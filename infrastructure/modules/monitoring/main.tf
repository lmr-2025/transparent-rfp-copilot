...
resource "aws_cloudwatch_metric_alarm" "high_error_rate" {
  count = var.create_app_alarms && var.alb_arn_suffix != "" ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "rds_high_cpu" {
  count = var.create_db_alarms && var.rds_instance_id != "" ? 1 : 0
...
resource "aws_cloudwatch_composite_alarm" "service_unhealthy" {
  ...
  alarm_rule = join(" OR ", compact([
    var.alb_arn_suffix != "" ? "ALARM(${aws_cloudwatch_metric_alarm.high_error_rate[0].alarm_name})" : "",
    var.alb_arn_suffix != "" ? "ALARM(${aws_cloudwatch_metric_alarm.high_response_time[0].alarm_name})" : "",
  ]))
