...
// A skill group with optional draft preview
type SkillGroup = {
  id: string;
  type: "create" | "update";
  skillTitle: string;
  ...
  urls: string[];
  status: "pending" | "approved" | "generating" | "ready_for_review" | "reviewed" | "saving" | "done" | "error" | "rejected";
  error?: string;
  reason?: string;
  // Draft content after LLM generation
  draft?: DraftContent;
  // Original content (for updates, to show diff)
  originalContent?: string;
  originalTitle?: string;
  originalTags?: string[];
};

type WorkflowStep = "input" | "analyzing" | "review_groups" | "generating" | "review_drafts" | "saving" | "done";
...
export default function BulkUrlImportPage() {
  ...
  const generateDrafts = async () => {
    ...
    for (const group of approvedGroups) {
      setSkillGroups(prev => prev.map(g =>
        g.id === group.id ? { ...g, status: "generating" } : g
      ));
  ...
  const saveReviewedDrafts = async () => {
    ...
    for (const group of reviewedGroups) {
      setSkillGroups(prev => prev.map(g =>
        g.id === group.id ? { ...g, status: "saving" } : g
      ));
  ...
  // UI Helpers
  const toggleGroupApproval = (groupId: string) => {
    setSkillGroups(prev => prev.map(g => {
      if (g.id === groupId) {
        return { ...g, status: g.status === "approved" ? "pending" : "approved" };
      }
      return g;
    }));
  };
  ...
  const approveDraft = (groupId: string) => {
    setSkillGroups(prev => prev.map(g =>
  ...
  const rejectDraft = (groupId: string) => {
    setSkillGroups(prev => prev.map(g =>
      g.id === groupId ? { ...g, status: "rejected" } : g
    ));
  };
  ...
  const moveUrl = (fromGroupId: string, url: string, toGroupId: string) => {
    setSkillGroups(prev => {
      const newGroups = prev.map(g => {
        if (g.id === fromGroupId) return { ...g, urls: g.urls.filter(u => u !== url) };
        if (g.id === toGroupId) return { ...g, urls: [...g.urls, url] };
        return g;
      });
      return newGroups.filter(g => g.urls.length > 0);
    });
  };

  const createNewGroupFromUrl = (fromGroupId: string, url: string, newTitle: string) => {
    setSkillGroups(prev => {
      const newGroups = prev.map(g => {
        if (g.id === fromGroupId) return { ...g, urls: g.urls.filter(u => u !== url) };
        return g;
  ...
  // Counts
  const pendingCount = skillGroups.filter(g => g.status === "pending").length;
  const approvedCount = skillGroups.filter(g => g.status === "approved").length;
  const readyForReviewCount = skillGroups.filter(g => g.status === "ready_for_review").length;
  const reviewedCount = skillGroups.filter(g => g.status === "reviewed").length;
    ...
    <div style={styles.container}>
      ...
      {workflowStep === "input" && (
        <div style={styles.card}>
          <h3 style={{ marginTop: 0 }}>Paste Documentation URLs</h3>
          <textarea
            value={urlInput}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {skillGroups.map((group) => (
              <div key={group.id} style={{ border: `1px solid ${getGroupStatusStyle(group.status).borderColor}`, borderRadius: "8px", backgroundColor: getGroupStatusStyle(group.status).backgroundColor, overflow: "hidden" }}>
                <div style={{ padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }} onClick={() => toggleExpand(group.id)}>
                        ...
                        {group.status === "pending" && skillGroups.length > 1 && (
                          <select
                            onChange={(e) => {
                              ...
                              if (value === "new") {
                                const newTitle = prompt("Enter name for new skill:");
      ...
      {workflowStep === "generating" && (
        <div style={styles.card}>
          <LoadingSpinner title="Generating content..." subtitle="Creating skill content from URLs" />
                ...
                {previewGroup?.id === group.id && group.draft && (
                  <div style={{ borderTop: "1px solid #e2e8f0", padding: "16px", backgroundColor: "#f8fafc" }}>
                    ...
                    <div style={{ marginTop: "12px" }}>
                      <button
                        onClick={() => {
                          const newContent = prompt("Edit content (this is a simple edit - for complex edits, approve and edit in the library):", group.draft?.content);
                          if (newContent !== null) {
