...
interface GoogleSlidesPickerProps {
  ...
  disabled?: boolean;
  onFillComplete?: (data: { presentationId: string; webViewLink: string }) => void;
}

export function GoogleSlidesPicker({ context, disabled, onFillComplete }: GoogleSlidesPickerProps) {
  ...
  const {
    ...
    params: { q: searchQuery || undefined, limit: 30 },
    responseKey: "presentations",
    transform: (data) => (Array.isArray(data) ? data : []),
    retry: false,
    staleTime: 0, // Always consider data stale so refetch works
    gcTime: 0, // Don't cache results (new presentations appear immediately on refresh)
  });

  // isLoading = first load, isFetching = any fetch including refetch
  const presentationsLoading = isLoading || isFetching || isRefreshing;

  // Force refresh that invalidates cache
  const handleRefresh = useCallback(async () => {
    setIsRefreshing(true);
    try {
      // Invalidate the query cache first
      await queryClient.invalidateQueries({ queryKey: ["google-slides", searchQuery] });
      // Then refetch
      await refetchPresentations();
    } finally {
      setIsRefreshing(false);
    }
  }, [queryClient, searchQuery, refetchPresentations]);
