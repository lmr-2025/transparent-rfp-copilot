 ...
 *
 * @example
 * const { list, get, create, update, remove } = useApiResource<Skill, SkillCreate, SkillUpdate>({
 *   baseUrl: "/api/skills",
 *   singularKey: "skill",
 *   pluralKey: "skills",
 * });
 ...
 * const { data: skills } = list();
 * const { data: skill } = get(id);
 * create.mutate({ title: "New", content: "..." });
...
>(config: {
  ...
  const { baseUrl, singularKey, pluralKey, staleTime } = config;
  const queryKeys = {
    all: [pluralKey] as QueryKey,
    one: (id: string) => [singularKey, id] as QueryKey,
  };

  return {
    /** Fetch all items */
    list: (
      options?: Omit<ApiQueryOptions<TData[]>, "url" | "queryKey" | "responseKey">
    ) =>
      useApiQuery<TData[]>({
        queryKey: queryKeys.all,
        url: baseUrl,
        responseKey: pluralKey,
        staleTime,
        ...options,
      }),
    ...
    /** Create new item */
    create: useApiMutation<TData, TCreate>({
      url: baseUrl,
      method: "POST",
      responseKey: singularKey,
    ...
    update: useApiMutation<TData, { id: string; data: TUpdate }>({
      url: (vars) => `${baseUrl}/${vars.id}`,
      method: "PUT",
      responseKey: singularKey,
      invalidateKeys: [queryKeys.all],
    }),
